...
def run_backtest_v15():
    ...
    print("🚀 V15 생존 전략 테스트 시작 (2024-2025년)")
    print("📌 조건: K=0.7, 거래대금 200% 폭발, 지수 필터, 시총 상위 30%, 비용 1%")
    ...
    cap_cutoff = np.percentile(caps, 70) if caps else 0
    ...
    for filename in tqdm(file_list):
        ...
        try:
            ...
            # [조건 2] 거래대금 폭발 (당일 거래대금 > 최근 5일 평균의 200%)
            df['Value'] = df['Close'] * df['Volume']
            df['Value_MA5'] = df['Value'].rolling(window=5).mean()
            df['vol_spike'] = df['Value'] >= (df['Value_MA5'].shift(1) * 2.0)
