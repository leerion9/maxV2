...
- 시간 기준: KIS 응답 헤더 `Date`를 이용해 서버-로컬 오프셋을 추정하고, 주요 스케줄(유니버스 준비/청산/종료)은 해당 기준 시각(KST)으로 판단하도록 개선.
- 청산 시각: 익일 시가 청산을 **08:55(KST) 시장가 주문**으로 변경(장중 기동 시 전일 보유 즉시 청산 로직은 유지).
